...
import com.cemk.exp.sendMail.SendAttachmentMail;
...
public class TimerClass {
	...
	public static void scheduledBackup(int hour, int min) {
		...
		logger.info("Timer will start after: "
				+ DateUtil.getHourMinSecond(milliseconds));
		// long delay = 1000;
		timer.schedule(new TimerTask() {
			public void run() {
				...
				if (!backup.exists())
					backup.mkdirs();
				boolean deleteStatus = CommonUtil.deleteAllFiles(path, ".sql");
				if (!StringUtils.isEmpty(username)
						&& !StringUtils.isEmpty(password)
						&& !StringUtils.isEmpty(databaseName)
						&& !StringUtils.isEmpty(path) && deleteStatus) {
					if (TimerClass.backupDB(username, password, databaseName,
							backup)) {
						logger.info("+--------------------------------------+");
	...
	public static boolean backupDB(String dbUserName, String dbPassword,
			String dbName, File path) {
		String outputFileName = dbName + "_"
				+ DateUtil.getFormatedCurrentDate("dd-MMM-yyyy@HH-mm-ss")
				+ ".sql";
		String executeCmd = "mysqldump --user=" + dbUserName + " --password="
				+ dbPassword + " " + dbName + " -r " + outputFileName;
		...
		try {
			...
			if (processComplete == 0) {
				...
				logger.info("+----------------------------------------------------+");
				outputFileName = System.getProperty("user.home")
						+ File.separator + "Ihalkhata Data Backup"
						+ File.separator + outputFileName;
				/*SendAttachmentMail.getInstance().sendAttachmentEmail(
						SendMailConstants.admin, "BackUp File", outputFileName);*/
				return true;
			} else {
				logger.info("+--------------------------------------+");
