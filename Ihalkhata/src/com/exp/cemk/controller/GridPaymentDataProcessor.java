...
public class GridPaymentDataProcessor {
	...
	public JSONObject getGridPaymentData(int start, int limit, int groupId) {
	...
	public JSONObject getSelectedPaymentGrid(int start, int limit, int groupId) {
		int reccnt = 0;
		// Connection con = null;
		// Statement stmt = null;
		// ResultSet rs = null;
		...
		try {
			...
			for (int i = 0; i < paymentList.size(); i++) {
				if (paymentList.get(i).getAmount() > 0) {
					jsonItems.put("amount", CommonUtil
							.getRoundedValue(paymentList.get(i).getAmount()));
	...
	public JSONObject getHomePaymentDetails(int groupId, String userName) {
		double debtAmount = 0;
		...
		try {
			...
			jsonObject.put("debit",
					"Rs." + CommonUtil.getRoundedValue(debtAmount));
			jsonObject.put("credit",
					"Rs." + CommonUtil.getRoundedValue(creditAmount));
			String gainer = "", loser = "";
			...
			expenditureList = expenditureData.getExpenditureData(month, year,
					month, year, "5", groupId);
			String expense = "Not Available";
			for (int i = 0; i < expenditureList.size(); i++) {
				if (userName.equals(expenditureList.get(i).getUserId())) {
					expense = CommonUtil.getRoundedValue(
							expenditureList.get(i).getAmount()).toString();
				}
			}
			jsonObject.put("expense", " Rs."+expense);
			// System.out.println("Data found in ID Query = "+checkFlag);
