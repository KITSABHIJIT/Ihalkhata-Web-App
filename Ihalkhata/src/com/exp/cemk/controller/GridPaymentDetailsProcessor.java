...
public class GridPaymentDetailsProcessor {
	...
	public JSONObject getGridPaymentDetails(int start, int limit, int groupId) {
	...
	public JSONObject getDebtPaymentDetails(int start, int limit, int groupId,
			String userId) {
	...
	public JSONObject getSelectedPaymentDetailsGrid(int start, int limit,
		...
		try {
			...
			for (int i = paymentDetailsList.size() - 1; i >= 0; i--) {
				jsonItems
						.put("amount", CommonUtil
								.getRoundedValue(paymentDetailsList.get(i)
										.getAmount()));
	...
	public JSONObject getDebtCreditNotifications(int start, int limit,
			int groupId, String userId, String type) {
		int reccnt = 0;
		int end = start + limit;
		...
		try {
			...
			for (int i = 0; i < paymentDetailsList.size(); i++) {
				...
				String detailedDesc = paymentDetailsList.get(i).getDesc();
				String desc = "";
				String groupDesc = "";
				if (!CommonUtil.isNullorEmpty(detailedDesc)) {
					desc = detailedDesc.split("[|]")[0];
					groupDesc = detailedDesc.split("[|]")[1];
				}
				...
				if ("1".equals(type)) {
					...
					jsonItems.put(
							...
							marker
									...
									+ "Rs."
									+ CommonUtil
											.getRoundedValue(paymentDetailsList
													.get(i).getBalance())
									+ "</b> for <b>"
